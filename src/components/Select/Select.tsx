...
export const Select: FC<PopoverProps> = ({ items, style = {} }) => {
  ...
  return (
    <div>
      <div ref={setReferenceElement} onClick={handleClick} className="w-fit cursor-pointer">
        <div
          className="flex justify-between items-center w-154 h-36 outline-none focus:outline-hidden text-14 border-1 border-gray4 pl-15 pr-10 rounded-4 mr-10 mb-10"
          style={style}
        >
          {items[selectValue]}
      ...
      {visible &&
        createPortal(
          <div ref={setPopperElement} className="pointer-events-auto" style={{ ...styles.popper, zIndex }} {...attributes.popper}>
            <div
              className="w-154 h-fit bdrs:10px border-1 border-gray4 translate-x-_5 my-5 bg-light rounded-4 overflow-hidden"
